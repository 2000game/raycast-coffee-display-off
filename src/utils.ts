...
export async function startCaffeinateDisplayOff(updates: Updates, hudMessage?: string, additionalArgs?: string) {
  if (hudMessage) {
    await showHUD(hudMessage);
  }
  await stopCaffeinate({ menubar: false, status: false });
  // -i flag: prevent system from idle sleeping (display can turn off, but system stays awake)
  const args = additionalArgs ? `-i ${additionalArgs}` : "-i";
  exec(`/usr/bin/caffeinate ${args} || true`);
  await update(updates, true);
}
